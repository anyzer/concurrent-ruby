...
module Concurrent
  ...
  # outer action.
  #
  # Nested actions must always be called using the `agent` argument passed to
  # the block/proc/lambd (the `self` reference within an action does *not*
  # refer to the Agent). This is slightly different from Clojure, which does not
  # pass the agent into the action function. Passing the Agent as a block
  # argument in Ruby allows the runtime to more efficiently manage the closure
  # in which the action is dispatched.
  #
  ...
  class Agent < Synchronization::Object
    ...
    AWAIT_ACTION = ->(agent, value, latch){ latch.count_down; AWAIT_FLAG }
    ...
    #   is not {#failed?}.
    #
    #   The action must be a block/proc/lambda which takes 2 or more arguments.
    #   The first argument is a reference to the Agent itself (`self` will not
    #   work within the action). The second argument is the {#value} of
    #   the Agent. Any arguments passed to the send method via the `args`
    #   parameter will be passed to the action as the remaining arguments. The
    #   action must return a new value for the Agent.
    #
    ...
    #   @yieldparam [Concurrent::Agent] agent a reference to `self`
    ...
    def execute_next_job
      ...
      new_value = job.action.call(self, old_value, *job.args)
      @caller.value = nil
